...
> **참고**  
> - 본 프로젝트에서 사용되는 **LLM 기반 멀티에이전트 시스템**은 GPT 4o-mini를 활용합니다.  
> - 시장 상태를 결정하는 것은 **멀티에이전트**의 몫입니다. 멀티에이전트가 결정한 최종 시장 상태(예: Up/Down/Flat 등)에 따라, LSTM이 해당 상태에 **Attention**을 두고 그에 맞춰 예측을 수행하는 구조입니다.
...
1. **Generator (생성기)**  
   ...
   - 에이전트가 결정한 *최종 시장 상태*를 임베딩하여, 해당 시나리오에 맞는 주가 변동률을 생성  
...
결정된 시장 상태(Up/Down/Flat)는 곧 **LSTM + Attention** 모듈이 예측을 수행할 때 참고하는 **핵심 파라미터**로 활용됩니다.  
...
아래 예시에서는 삼성전자(005930.KS)를 대상으로, 2014년 12월 1일부터 2024년 12월 1일까지의 데이터를 다운로드하고, 이 중 2024년 9월 26일 이전까지의 구간을 학습 데이터로 사용하였습니다. 2024년 9월 27일부터 1주 기간은 예측 범위로 설정했습니다.
...
예측 시점 직전 일주일의 뉴스를 분석한 결과 멀티 에이전트의 판단은 '하락'이었고, 이를 바탕으로 예측을 진행하였습니다.
...
예측 결과는 위와 같습니다.
MSE와 RMSE를 통해 모델을 평가하긴 했지만, 당초 목표는 멀티 에이전트의 판단을 얼마나 잘 반영하는 예측값을 만들 수 있는지에 있었습니다.
다행히 결과를 종합해보면, 에이전트 판단을 꽤 잘 반영하고 있음을 확인할 수 있었습니다.
